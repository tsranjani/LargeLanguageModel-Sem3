...
INPUT_FILE = r"C:\Users\wmdna\Desktop\Scraping\Success - Copy\Business_Lodgings\all_lodgings_sweden.json"
OUTPUT_FILE = r"c:\Users\wmdna\Desktop\Scraping\Success - Copy\Business_Lodgings\all_lodging_sweden_flat_new.json"
...
def extract_opening_hours(value):
    """Parse schema:openingHoursSpecification into a clean structure."""
    if not value:
        return None
    if isinstance(value, dict):
        value = [value]
...
def flatten_graph(graphs):
    """Flatten LodgingBusiness @graph into a single record."""
    main = next((g for g in graphs if g.get("@type") == "schema:LodgingBusiness"), None)
    if not main:
        return None
    ...
    flat = {
        ...
        "type": main.get("@type"),
        "additional_type": extract_id(main.get("schema:additionalType")),
        ...
        "telephone": main.get("schema:telephone"),
        "opening_hours": extract_opening_hours(main.get("schema:openingHoursSpecification")),
        "checkin_time": main.get("schema:checkinTime"),
        "checkout_time": main.get("schema:checkoutTime"),
        "star_rating": main.get("schema:starRating"),
        "aggregate_rating": main.get("schema:aggregateRating"),
        "price_range": main.get("schema:priceRange"),
        ...
        "audience": extract_id(main.get("schema:audience")),
        "contains_place": extract_id(main.get("schema:containsPlace")),
    ...
    # Replace empty values with None for cleanliness
...
def main():
    ...
    print(f"✅ Flattened {len(flattened)} lodging entries.")
