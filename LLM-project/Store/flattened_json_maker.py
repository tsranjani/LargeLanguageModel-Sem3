...
# --- File paths ---
INPUT_FILE = r"c:\Users\wmdna\Desktop\Scraping\Success - Copy\Store\all_store_sweden.json"
OUTPUT_FILE = r"c:\Users\wmdna\Desktop\Scraping\Success - Copy\Store\all_stores_sweden_flat.json"
...
def flatten_graph(graphs):
    """Flatten Store @graph into a single record."""
    main = next((g for g in graphs if g.get("@type") == "schema:Store"), None)
    if not main:
        return None
    ...
    flat = {
        ...
        "type": main.get("@type"),
        "additional_type": extract_id(main.get("schema:additionalType")),  # e.g. schema:BookStore, schema:ToyStore
...
def main():
    ...
    print(f"✅ Flattened {len(flattened)} store entries.")
